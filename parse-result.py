...
def output_html():
    ...
    RETRANSMIT_LEGEND = (
        "<P></P>"
        "<div><table class=\"legend2\">",
        "    <tr><td colspan=\"2\"><p>Legend</p></td></tr>",
        "<tr>",
        "    <td><div style=\"padding: 5px; margin: 1px; background-color: #99FF99\"></div></td>",
        "    <td class=\"legend2\">Percentage of extra bytes sent &lt; 0.1%</td>",
        "</tr>",
        "<tr>",
        "   <td><div style=\"padding: 5px; margin: 1px; background-color: #FFFF99\"></td>",
        "    <td class=\"legend2\">Percentage of extra bytes sent &ge; 0.1% and &lt; 0.4%</td>",
        "</tr>",
        "<tr>",
        "   <td><div style=\"padding: 5px; margin: 1px; background-color: #ff6666\"></td>",
        "    <td class=\"legend2\">Percentage of extra bytes sent &ge; 0.4%</td>",
        "</tr></table></div>",
        "<P></P>"
    )
    ...
    outfile.writelines("\n<!-- Retransmit result table below -->\n")
    outfile.writelines("<table class=\"result\">\n")
    outfile.writelines("<tr><th>Percentage of extra bytes sent</th>\n")
    for desthost in sorted(bw_array.iterkeys()):
        # check if we are using IP address or hostname/FQDN
        try:
            socket.inet_aton(desthost)
    ...
    for srchost in sorted(bw_array.iterkeys()):
        ...
        for desthost in sorted(bw_array.iterkeys()):
            if srchost == desthost:
                ...
                retransmit_pct = round(bw_array[srchost][desthost]["sent"]["bytes"] / bw_array[srchost][desthost]["received"]["bytes"],10)
                outfile.writelines("<td class=")
                if retransmit_pct < 1.001:
                    outfile.writelines("\"green\"")
                elif retransmit_pct >= 1.004:
                    outfile.writelines("\"red\"")
                else:
                    outfile.writelines("\"yellow\"")
                outfile.writelines(">")
                outfile.writelines( '%g'%(round((retransmit_pct-1)*100,2))+"%" )
                outfile.writelines("</td>")
        outfile.writelines("</td></tr>\n")
    outfile.writelines("</table>\n\n")

    for line in RETRANSMIT_LEGEND:
        outfile.writelines(line + "\n")
