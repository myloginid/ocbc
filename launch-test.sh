...
function cleanup {
    if [ "$COMPLETED" == "1" ]; then
            ...
            ssh -o StrictHostKeyChecking=no "$OPT_USER@$SERVER" "ps aux | grep 'iperf3' | awk '{print \$2}' | xargs kill" > /dev/null 2>&1
...
for i in "${!HOSTS[@]}"; do
        ...
        for j in "${!HOSTS[@]}"; do
                ...
                if [ "$SERVER" != "$CLIENT" ]; then
                        ...
                        ssh -o StrictHostKeyChecking=no "$OPT_USER@$SERVER" "$CMD_IPERF_SERVER" > /dev/null 2>&1 &
                        ...
                        ssh -o StrictHostKeyChecking=no "$OPT_USER@$CLIENT" "$clientcmd" >> "$OUTFILE"
			echo "}," >> "$OUTFILE"
                fi
        done
        echo "]" >> "$OUTFILE"
